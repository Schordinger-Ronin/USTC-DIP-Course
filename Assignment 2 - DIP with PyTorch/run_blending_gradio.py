...
def create_mask_from_points(points, img_h, img_w):
    """
    Creates a binary mask from the given polygon points using PIL.
    """
    # 1. Create an all-black single-channel image with the same dimensions as the original image (mode 'L' means 8-bit grayscale)
    mask_img = Image.new('L', (img_w, img_h), color=0)
    
    # 2. Get the drawing context
    draw = ImageDraw.Draw(mask_img)
    
    # 3. Convert point set format
    # points is currently a numpy array, PIL requires it to be a list of tuples [(x1, y1), (x2, y2), ...]
    points_list = [tuple(pt) for pt in points]
    
    # 4. Draw the polygon and fill the interior with 255 (white)
    draw.polygon(points_list, outline=255, fill=255)
    
    # 5. Convert the PIL image to a numpy array losslessly
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # 1. Define a 3x3 Laplacian kernel
    laplacian_kernel = torch.tensor([[0.0, 1.0, 0.0],
                                     [1.0, -4.0, 1.0],
                                     [0.0, 1.0, 0.0]], device=device)
    kernel = laplacian_kernel.view(1, 1, 3, 3).repeat(3, 1, 1, 1)
    
    # 2. Perform convolution on the entire images separately to extract high-frequency gradient information
    fg_grad = F.conv2d(foreground_img, kernel, padding=1, groups=3)
    blend_grad = F.conv2d(blended_img, kernel, padding=1, groups=3)
    
    # 3. Core fix: Expand the Mask to 3 RGB channels and convert to boolean (True/False)
    fg_mask_bool = foreground_mask.expand(-1, 3, -1, -1).bool()
    bg_mask_bool = background_mask.expand(-1, 3, -1, -1).bool()
    
    # 4. Use boolean indexing to extract the pixels inside the Mask into a 1D tensor
    # As long as your polygon is not moved outside the boundary of the background image, the lengths of these two tensors will definitely be equal!
    fg_grad_masked = fg_grad[fg_mask_bool]
    blend_grad_masked = blend_grad[bg_mask_bool]
    
    # 5. Calculate the difference in internal gradients and sum them up
    diff = (blend_grad_masked - fg_grad_masked) ** 2
    loss = diff.sum()
    ...
    with gr.Row():
        with gr.Column():
            ...
            foreground_image_with_polygon = gr.Image(
                label="", type="pil", interactive=False, height=300
            )
            gr.Markdown(
